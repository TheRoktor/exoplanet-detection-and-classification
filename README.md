...
### 3.1 Correlations
![Correlation matrix](data/corrs.png) Correlation matrix

After computing the correlation matrix from the imputed data, we can see that not many features have strong correlations; most being close to, or exactly zero. Strong correlations are only seen in 'related features' i.e. features measuring uncertainties (_err1 and _err2 features.) These are  1-σ uncertainties, and as such the correlation between these will always be high. Outside of those few correlations, nothing suggests any potential multicollinearity problems. "Such problems would not necessarily affect the accuracy of the model, but they would affect the weights (importances) of certain features. This problem was resolved by using two different methods of ranking. The decision tree was especially useful, as more than one could be constructed ensuring any possible artifacts regarding the previously mentioned feature weights would not cause innacuracies.
...
After the missing data had been taken care of it was time to begin plotting it in order to see, whether certain clusters would appear, thus making in much more likely to be able to build a prediction model. Ideally we'd like three clusters; one containing only 'FALSE POSITIVES', one 'CONFIRMED' and the remaining one 'CANDIDATES'.
...
![Plotted data in respect to first 2 principal components](data/PCAplot.PNG) Scatter plot of all datapoints
...
![Clustered datapoints](data/clustering.PNG) Scatter plot of all clusters
...
In the graph below the relationship between the number of features and percentage of variance kept can be seen (27 PCs represent 95% of the total variance).
![Variance graph](data/pca.PNG) Graph showing the relationship between number of PCs and variance
